...
const Paquetes = () => {
  ...
  const subcategoryPackage = information.reduce((acc, offer) => {
  ...
  return (
    <div className="offers-container">
      ...
      <div className="package-container">
        <h3>Compra de paquetes</h3>
        <hr />
      </div>
      <div className="Offer-content">
        {Object.keys(subcategoryPackage).map((subcategory) => (
          <div className="subcategoryTitle" key={subcategory}>
            <h4>{subcategory}</h4>
            <div className="cards">
              {subcategoryPackage[subcategory].map((offer) => (
                <Card
                  key={offer._id}
                  id={offer._id}
                  image={offer.img}
